...
This page explains how to use `templates/notebooks/fabric_data_product_mvp.py` as a safe, copy-pasteable starter for Microsoft Fabric.

The default mode is `DRY_RUN = True`, so a first run does not write production outputs.
...
5. Run all cells to validate this sequence: profile → DQ → governance → lineage → handover.
...
13. Handover framework pack (Copilot prompt block included) plus final run summary cell.

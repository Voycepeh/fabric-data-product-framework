...
This page explains how to use `templates/notebooks/fabric_data_product_mvp.py` as a safe, copy-pasteable starter for Microsoft Fabric. The default mode is `DRY_RUN = True` so a first run does not write production outputs.
...
5. Run all cells to validate profile → DQ → governance → lineage → handover flow.
6. Replace the transformation section marker with domain logic.
7. Replace Fabric adapters (`fabric_reader`, `fabric_writer`) only after dry run succeeds.
...
12. AI generated lineage and transformation summary (**Copilot prompt block included**)
13. Handover framework pack (**Copilot prompt block included**) + final run summary cell
...
The template includes a dedicated section clarifying:
- **Human fills this in**: business context, source/target bindings, transformation logic, approvals.
- **Framework generates this**: profiles, DQ execution outputs, governance suggestions, lineage shape, validation checks.

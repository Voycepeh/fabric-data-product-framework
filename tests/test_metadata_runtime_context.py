...
def test_runtime_context_reads_javamap_keys(monkeypatch):
    context = JavaMapLike(
        {
            "currentNotebookName": "01_da_agreement_template",
    ...
    assert out["currentNotebookName"] == "01_da_agreement_template"
...
def test_register_current_notebook_uses_current_and_fallback_keys(monkeypatch):
    ...
    assert row["notebook_name"] == "01_da_agreement_template"
...
def test_templates_do_not_import_root_private_helpers():
    nb = Path("templates/notebooks/01_da_agreement_template.ipynb").read_text(encoding="utf-8")
    data = json.loads(nb)

...
USAGE_GUIDE_FILE = ROOT / "docs" / "reference" / "function-usage-guide.md"
...
def test_reference_includes_callable_finder_block() -> None:
    ...
    assert content.index("## Find a callable") > content.index("# Function Reference")


def test_function_usage_guide_is_generated_with_orientation_sections() -> None:
    generate_reference()
    content = USAGE_GUIDE_FILE.read_text(encoding="utf-8")
    assert "# Function Usage Guide" in content
    assert "## Start from the templates" in content
    assert "## What runs where" in content
    assert "## Find a callable" not in content

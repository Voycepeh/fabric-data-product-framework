...
def test_run_data_product_calls_schema_drift_wrappers(monkeypatch):
    ...
    monkeypatch.setattr("fabric_data_product_framework.quality.load_latest_schema_snapshot", lambda *args, **kwargs: None)

    def _check_schema_drift(**kwargs):
        seen["check"] += 1
        return {"status": "no_baseline", "can_continue": True}

    monkeypatch.setattr("fabric_data_product_framework.quality.check_schema_drift", _check_schema_drift)
    monkeypatch.setattr("fabric_data_product_framework.quality.build_and_write_schema_snapshot", lambda **kwargs: {"written": True})
...
def test_data_drift_second_run_uses_written_baseline(monkeypatch):
    ...
    monkeypatch.setattr("fabric_data_product_framework.quality.load_latest_partition_snapshot", _load_latest_partition_snapshot)
    monkeypatch.setattr("fabric_data_product_framework.quality.build_and_write_partition_snapshot", _build_and_write_partition_snapshot)
    monkeypatch.setattr("fabric_data_product_framework.quality.check_partition_drift", _check_partition_drift)

...
from fabricops_kit.config import setup_fabricops_notebook
...
def _sample_config():
    from fabricops_kit.config import AIPromptConfig, FrameworkConfig, GovernanceConfig, LineageConfig, NotebookRuntimeConfig, PathConfig, QualityConfig
...
def test_setup_uses_current_run_id_and_user_fallback(monkeypatch):
    ...
    out = setup_fabricops_notebook(config=_sample_config(), env="Sandbox", check_ai=False)

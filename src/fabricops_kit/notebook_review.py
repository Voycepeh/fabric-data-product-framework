"""Fabric notebook human-in-the-loop review UI workflows.

This module provides widget-based review interfaces used during notebook
execution when AI suggestions must be reviewed and approved by humans before
deterministic pipeline enforcement. It currently supports DQ rule approval and
DQ rule deactivation review.

Future extension points include classification review, business context capture,
data agreement and steward metadata capture, and governance sign-off workflows.
"""
...
def review_dq_rules(candidate_rules, table_name: str):
    ...
    candidate_rules : list[dict]
        Candidate rule dictionaries extracted from AI responses. AI suggestions are advisory only.
    table_name : str
    ...
    -------
    None
        Displays an interactive widget where humans approve/reject before pipeline enforcement.

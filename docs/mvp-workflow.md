...
| 2. Setup config and environment | Starter-kit led | Provide runtime inputs | Build runtime context and checks | N/A | Runtime config printed/validated | `runtime_config` |
| 3. Declare source and ingest data | Starter-kit led | Select synthetic source | Read source via adapters | N/A | Source read succeeds | `source_declaration` |
| 4. Profile source and capture metadata | Starter-kit led | Trigger profiling | Build and persist profile | Summarize profile findings | Metadata profile rows stored | `source_profile` |
...
| 7. Build transformation pipeline | Starter-kit led | Execute transform | Apply helpers and write output | Suggest code improvements | Output write or DRY_RUN equivalent | `output_table` |

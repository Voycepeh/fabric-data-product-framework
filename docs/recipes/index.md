...
This proves the core flow: generated sample DataFrame -> persisted source table -> proposal and evidence captured in `02_ex` -> approved contract metadata loaded/enforced in `03_pc` -> valid/quarantine split -> output write.
...
In this sample flow, there is no checked-in sample CSV or pre-seeded contract file: `02_ex` captures proposal evidence and candidate updates; governance approval is finalized in `01_data_sharing_agreement`, then `03_pc` consumes approved metadata for enforcement.
